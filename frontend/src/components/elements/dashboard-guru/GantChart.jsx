...
const GantChart = ({ schedules, durasi }) => {
  ...
  const calculatePercentage = (start, end) => {
    ...
    return (totalMinutes / durasi) * 100; // Convert to percentage
  };
  ...
  const calculateLeft = (start) => {
    const [startHour, startMinute] = start.split(":").map(Number);
    const startTime = startHour * 60 + startMinute;
    return ((startTime - 420) / durasi) * 100; // 420 = 07:00 in minutes
  };
    ...
    <div className="overflow-x-auto">
      {days.map((day) => (
        ...
        >
          ...
          <div className="relative   sm:col-span-9 border-b border-l  border-gray-100 h-14    sm:h-10">
            {schedules
